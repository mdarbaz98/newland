<?php
$sql = 'SELECT * FROM `tmp_cart_tbl` WHERE id="'.$id.'"';
$result = $dbstripepay->query($sql);
$row = $result->fetch_assoc();

$sql1 = 'select * from drg_billing_info where c_id="'.$row['id'].'"';	
$result1 = $dbstripepay->query($sql1);
$rowsadd = $result1->fetch_assoc();
$toemail = mb_convert_encoding($rowsadd['bemail_address'], "UTF-8", "UTF-8");
$orderid = "#INV-".$row['id']."".substr(strtotime($row["timestamp"]), 0, 4);
$ocdate = date('d-M, Y', strtotime($row['timestamp']));

$tids = array();
$tsql = "SELECT * FROM `tbl_traking_ids` WHERE order_id='".$id."'";
$tresult = $dbstripepay->query($tsql);
...
$cart_items = json_decode($row['cart_items']);
	...
	<tbody>
		...
		<tr>
			<td colspan="2" align="center">
				...
				<br>
				 ...
				 <h4 style="background-color: #e5e5e5;padding: 5px; width: fit-content;border-radius: 25px;border: 2px solid #bcbcbc;color:#808080;">ORDER ID: '.$orderid.'</h4>
				 
				 
				 <a href="https://oneglobalpharma.com/intermediate/cust_del_complete.php?web=stripepay&id='.$id.'" style="background-color: #63ffdb;padding: 5px 10px;border: 2px solid #00e3d9;border-radius: 25px;font-weight:600;color:#000;text-decoration:none;">Yes, i received my order</a><br><br>
				 
				 <a href="https://oneglobalpharma.com/intermediate/cust_not_recv_order.php?web=stripepay&id='.$id.'" style="background-color: #63ffdb;padding: 5px 10px;border: 2px solid #00e3d9;border-radius: 25px;font-weight:600;color:#000;text-decoration:none;">I have not received my order yet</a><br><br>
				 
				 <a href="https://oneglobalpharma.com/intermediate/miss_medication_recv.php?web=stripepay&id='.$id.'" style="background-color: #63ffdb;padding: 5px 10px;border: 2px solid #00e3d9;border-radius: 25px;font-weight:600;color:#000;text-decoration:none;">Received incomplete/incorrect/duplicate or expired medication</a>
				 <br>
				 <br>
			</td>
		</tr>
		...
		$msg .='</tr>
				...
				<tr>
					...
					<br><br>
						<a href="https://oneglobalpharma.com/intermediate/oc_sts_report.php?web=stripepay&sts=Delivered&id='.$id.'" style="padding: 10px 20px;
					...
					<br>
						<table width="100%" style="text-align:center;">
							<tr>
								...
								<td width="30%">
									<p>
									<a href="https://oneglobalpharma.com/intermediate/rep_an_issue.php?web=stripepay&id='.$id.'" style="	border:2px solid #0085eb;
														padding: 22px;
														color: #0085eb;
														font-size: 17px;
														text-decoration: none;
...
sendmail("admin@selcoenterprises.com", $row['web'], $toemail, "Your Order Delivered Successfully | ".$row['web'], $msg);
?>