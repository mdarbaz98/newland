...
class GloPharmamedz
{
	...
	function get_allorders_today($array, $inc) {
		...
		while($row = $result->fetch_assoc()) {
			...
			$arr['oid'] = $row['id'];
			$arr['orderValue'] = $row['order_value'];
	...
	function update_order_status_todayoc($data, $array) {
        ...
        $dbhost="162.251.80.212";
        $dbuser="globa7ox_globalProdAll";
        $dbpass="CxqD-Cm8m.^K";
        $dbname="globa7ox_globalProdAll";
        
        $conn = mysqli_connect($dbhost, $dbuser, $dbpass, $dbname);
        $sqlNew1 = "UPDATE orderdetails SET orderStatus='".$data->statusval."' WHERE orderid='".$orderno."'";
