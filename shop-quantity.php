<?php 
    ...
    while($row=$select_stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $output1 = "
            <label class='switch'>
                <input type='radio' class='{$status} listQuantity' id='quantity' data-product-name='{$productName}' data-product-image='{$productImage}' data-product-type='{$type}' data-strength-id='{$strength}' data-strength='{$strength_amount}' data-quantity-id='{$row['quantityCode']}' data-quantity='{$row['quantity']}' data-price='{$row['price']}' data-product-code='{$row['productCode']}' name='quantity' onClick='cartStatus(this)' value='{$row['quantity']}'>
                <span class='sliders {$class}'>
                    <span class='qty'>{$row['quantity']}<br><small>{$type}</small></span>
