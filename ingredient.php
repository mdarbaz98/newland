<?php 
    ...
    $ingredientName = ucwords(str_replace('-', ' ',$_GET['ingredientName']));
    $per_page=12;
    $start=0;
    $current_page=1;
    if(isset($_GET['page'])){
                ...
                <section class="pt-4">
                    <!--<?php echo $productSlugNew; ?>-->
                        <div class="row mx-n2">
                            ...
                            $select_product=$conn->prepare("SELECT * FROM ogproduct INNER JOIN ogproductingredient ON ogproduct.productCode=ogproductingredient.productCode WHERE ogproductingredient.productIngredientName = ?");
                            $select_product->execute([$ingredientName]);  
