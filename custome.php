<?php 
    ...
    $proSlug = $_GET['proslug'];
    $select_product=$conn->prepare("SELECT * FROM custom_prod WHERE slug = ?");
    $select_product->execute([$proSlug]);
    while($row=$select_product->fetch(PDO::FETCH_ASSOC)) {
        $pageName = $row['name'];
        $products = $row['products'];
    }
    $code = explode(",",$products);
    $products = "'".implode("','",$code)."'";
    // $productCategorySlug = $proSlug;
    // $prductCategoryModify = str_replace("-"," ",$productCategorySlug);
    // $productSlugNew = ucwords($prductCategoryModify);
    
    $productSeoTitle = $pageName;
    $productSeoDescription = $pageName;
...
<main class="shop-page offcanvas-enabled" style="padding-top: 89px; background: rgba(0,0,0,0);">
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 shop-products col-12 px-0 px-lg-2" style="padding-top:30px;padding-bottom:7px;">
                <section class="pt-4">
                    <!--<?php echo $productSlugNew; ?>-->
                        <h3 class="text-center"><?php echo ucwords($proSlug) ?></h3>
                        <div class="row mx-n2">
                            <?php
                            ...
                            $select_product=$conn->prepare("SELECT * FROM ogproduct WHERE productCode IN ($products)");
