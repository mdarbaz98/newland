...
if (strlen($addressId) > 0)
{
    ...
    if ($updateCusomer)
    {
        ...
        if (1 == 1)
        {
            ...
            while ($row = $selectCartProduct->fetch(PDO::FETCH_ASSOC))
            {
                ...
                $insertProduct = $conn->prepare("INSERT into ogorderproduct(shippingCompany, productName, productCategory, strength, orderid, productCode, quantityCode, strengthCode, quantity, quantityPrice, totalQuantity, totalPrice, total, discount, saveAmount, orgPrice, userId) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                $insertProduct->execute([$shippingCompany, $productName, $productCategory, $strengthName, $orderId, $productCode, $quantityCode, $strengthCode, $quantity, $quantityPrice, $totalQuantity, $orgPriceCart, $orgPriceCart * $totalQuantity, $discountCart, $saveAmountCart, $orgPriceCart, $userid]);
            }
    ...
    CURLOPT_URL => 'https://selco.newlandpharmacy.co.uk/api/v1/newland/helper/sendOrderConfirmationEmail',
    ...
    CURLOPT_POSTFIELDS =>'{
        "orderid":"'.$orderId.'",
        "key":"E41FDCAEAE335SOHAIL69BITCH26C97CD1C21F"
    }',
